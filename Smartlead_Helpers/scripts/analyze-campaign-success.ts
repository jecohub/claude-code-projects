...
interface CliArgs {
  ...
  verbose: boolean;
  detailed: boolean; // Fetch all leads for full categorization (slower)
  delayMs: number; // Delay between campaigns to avoid rate limits
}
...
function parseArgs(): CliArgs {
  ...
  const clientId =
    args.find((a) => a.startsWith("--clientId="))?.split("=")[1] || "77930";
...
function getCampaignUrl(campaignId: number): string {
...
function calculateCompositeScore(
  ...
  const normalizedSends = Math.min(
    (sent / NORMALIZATION.maxSent) * 100,
    100
  );
  const normalizedOpens = Math.min(
    openRate * NORMALIZATION.openRateMultiplier,
    100
  );
  const normalizedClicks = Math.min(
    clickRate * NORMALIZATION.clickRateMultiplier,
    100
  );
  const normalizedPositive = Math.min(
    positiveResponseRate * NORMALIZATION.positiveRateMultiplier,
    100
  );

  // Calculate weighted score
  const score =
    normalizedSends * weights.sends +
...
function buildReport(
  clientId: string,
  campaignMetrics: CampaignSuccessMetrics[],
  notes: string[]
): CampaignSuccessReport {
  ...
  if (aggregated.totalSent > 0) {
    aggregated.averageOpenRate = round(
      (aggregated.totalOpens / aggregated.totalSent) * 100
    );
    aggregated.averageClickRate = round(
      (aggregated.totalClicks / aggregated.totalSent) * 100
    );
    aggregated.averageReplyRate = round(
      (aggregated.totalReplies / aggregated.totalSent) * 100
    );
    aggregated.averagePositiveResponseRate = round(
      (aggregated.totalPositiveResponses / aggregated.totalSent) * 100
    );
  }
...
async function main() {
  ...
  const campaigns = campaignsResponse.items as any[];

  console.log(`Found ${campaigns.length} campaigns\n`);

  // Step 2: Process each campaign
  const campaignMetrics: CampaignSuccessMetrics[] = [];
  const notes: string[] = [];
  ...
  for (const campaign of campaigns) {
    ...
    if (args.verbose) {
      console.log(
        `[${processed}/${campaigns.length - skipped}] Processing: ${campaignName} (${campaignId})...`
      );
    } else {
      process.stdout.write(
        `\rProcessing campaigns: ${processed}/${campaigns.length - skipped}...`
      );
    }
    ...
    try {
      ...
      let categorization: LeadCategorization;
      let positiveResponses: PositiveResponses;
      ...
      const positiveResponseRate =
        sent > 0 ? (positiveResponses.total / sent) * 100 : 0;
      ...
      if (args.verbose) {
        console.log(
          `  -> Score: ${compositeScore}, Interested: ${positiveResponses.interested}, Sent: ${sent}`
        );
      }
  ...
  console.log(
    `    Total Positive Responses: ${report.aggregated.totalPositiveResponses.toLocaleString()}`
  );
  ...
  report.topPerformers.byCompositeScore.forEach((c, i) => {
    ...
    console.log(
      `       Score: ${c.compositeScore} | Interested: ${c.positiveResponses.interested} | Rate: ${c.positiveResponseRate}%`
    );
    console.log(
      `       ${c.campaignUrl}`
    );
  ...
  report.topPerformers.byPositiveResponse.forEach((c, i) => {
    ...
    console.log(
      `       Rate: ${c.positiveResponseRate}% | Interested: ${c.positiveResponses.interested} | Sent: ${c.engagement.sent}`
    );
    console.log(
      `       ${c.campaignUrl}`
    );
