...
import { calculateCampaignHealth, formatHealthSection, } from "../src/features/client-health/campaignHealthService.js";
function parseArgs() {
...
function formatReport(report, fromDateStr) {
    const textLines = [];
    // Summary section
    ...
    if (report.summary.totalLeads > 0) {
        textLines.push(`    - Not Started: ${report.summary.notStarted.toLocaleString()} (${((report.summary.notStarted / report.summary.totalLeads) *
            100).toFixed(1)}%)`);
        textLines.push(`    - In Progress: ${report.summary.inprogress.toLocaleString()} (${((report.summary.inprogress / report.summary.totalLeads) *
            100).toFixed(1)}%)`);
        textLines.push(`    - Completed: ${report.summary.completed.toLocaleString()} (${((report.summary.completed / report.summary.totalLeads) *
            100).toFixed(1)}%)`);
        textLines.push(`    - Blocked: ${report.summary.blocked.toLocaleString()} (${((report.summary.blocked / report.summary.totalLeads) *
            100).toFixed(1)}%)`);
        textLines.push(`    - Stopped: ${report.summary.stopped.toLocaleString()} (${((report.summary.stopped / report.summary.totalLeads) *
            100).toFixed(1)}%)`);
    }
    ...
    if (report.activeLeadsSummary.totalActive > 0) {
        textLines.push(`    - Not Started: ${report.activeLeadsSummary.notStarted.toLocaleString()} (${((report.activeLeadsSummary.notStarted /
            report.activeLeadsSummary.totalActive) *
            100).toFixed(1)}%)`);
        textLines.push(`    - In Progress: ${report.activeLeadsSummary.inprogress.toLocaleString()} (${((report.activeLeadsSummary.inprogress /
            report.activeLeadsSummary.totalActive) *
            100).toFixed(1)}%)`);
        textLines.push(`    - Blocked: ${report.activeLeadsSummary.blocked.toLocaleString()} (${((report.activeLeadsSummary.blocked /
            report.activeLeadsSummary.totalActive) *
            100).toFixed(1)}%)`);
        textLines.push(`    - Stopped: ${report.activeLeadsSummary.stopped.toLocaleString()} (${((report.activeLeadsSummary.stopped /
            report.activeLeadsSummary.totalActive) *
            100).toFixed(1)}%)`);
    }
