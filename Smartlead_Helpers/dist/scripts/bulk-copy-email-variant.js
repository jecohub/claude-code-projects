...
function parseArgs() {
    ...
    const clientId = args.find((a) => a.startsWith("--clientId="))?.split("=")[1] ||
        DEFAULT_CLIENT_ID;
    const sourceCampaignStr = args.find((a) => a.startsWith("--sourceCampaign="))?.split("=")[1];
    const sourceCampaign = sourceCampaignStr
        ? parseInt(sourceCampaignStr, 10)
        : DEFAULT_SOURCE_CAMPAIGN;
    const fromDateStr = args.find((a) => a.startsWith("--fromDate="))?.split("=")[1] ||
        DEFAULT_FROM_DATE;
    const fromDate = new Date(fromDateStr);
    const nameFilter = args.find((a) => a.startsWith("--nameFilter="))?.split("=")[1] ||
        DEFAULT_NAME_FILTER;
    return {
        execute,
        clientId,
...
async function findMatchingCampaigns(client, clientId, fromDate, nameFilter) {
    ...
    try {
        ...
        return matchingCampaigns.map((campaign) => ({
            id: campaign.id,
...
function displayPreview(campaigns, clientId, fromDate, nameFilter) {
    console.log("\n" + "=".repeat(60));
    ...
    console.log("Campaign ID".padEnd(15) +
        " | " +
    ...
    campaigns.forEach((campaign) => {
        ...
        console.log(String(campaign.id).padEnd(15) +
            " | " +
...
async function fetchSourceVariant(client, sourceCampaignId) {
    ...
    const sourceEmail = sourceSequences.find((seq) => seq.seq_number === SEQ_NUMBER);
    if (!sourceEmail) {
        throw new Error(`Email ${SEQ_NUMBER} not found in source campaign ${sourceCampaignId}`);
    }
    const variants = sourceEmail.sequence_variants || sourceEmail.seq_variants;
    if (!variants || !Array.isArray(variants)) {
        throw new Error(`Email ${SEQ_NUMBER} has no variants in source campaign ${sourceCampaignId}`);
    }
    const variantB = variants.find((v) => v.variant_label === VARIANT_LABEL);
    if (!variantB) {
...
function sanitizeSequences(targetSequences, targetEmailIndex, variantB) {
    return targetSequences.map((seq, index) => {
        const sanitized = {
        ...
        if (index === targetEmailIndex) {
            ...
            const variantA = existingVariants.find((v) => v.variant_label === "A");
            ...
            if (variants && Array.isArray(variants) && variants.length > 0) {
                sanitized.seq_variants = variants.map((variant) => ({
...
async function updateCampaign(client, campaignId, campaignName, variantB) {
    try {
        // Fetch target sequences
        const targetSequences = await client.getCampaignSequences(campaignId);
        // Find Email 2
        const targetEmailIndex = targetSequences.findIndex((seq) => seq.seq_number === SEQ_NUMBER);
        ...
        const sanitizedSequences = sanitizeSequences(targetSequences, targetEmailIndex, variantB);
        // Save sequences
...
async function executeUpdates(client, campaigns, sourceCampaignId) {
    console.log("\n" + "=".repeat(60));
    ...
    for (let i = 0; i < campaigns.length; i++) {
        ...
        process.stdout.write(`${progress} Updating campaign ${campaign.id}: ${campaign.name}... `);
        const result = await updateCampaign(client, campaign.id, campaign.name, variantB);
        results.push({
...
async function main() {
    ...
    try {
        ...
        const matchingCampaigns = await findMatchingCampaigns(client, args.clientId, args.fromDate, args.nameFilter);
        // Display preview
        displayPreview(matchingCampaigns, args.clientId, args.fromDate, args.nameFilter);
        ...
        if (args.execute) {
            ...
            console.log("=".repeat(60));
            console.log(`About to update ${matchingCampaigns.length} campaign(s) with Email ${SEQ_NUMBER} Variant ${VARIANT_LABEL}.`);
