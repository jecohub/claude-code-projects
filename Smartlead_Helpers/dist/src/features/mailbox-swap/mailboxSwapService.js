...
export class MailboxSwapService {
    ...
    async execute(params) {
        const { csvFilePath, clientId, fromDate, toDate, minReputation, maxReputation, activateCampaigns = false, removeExistingMailboxes = true, dryRun = true, } = params;
        // ── Step 1: Parse CSV ────────────────────────────────────────────────
        ...
        const accountByEmail = new Map(allClientAccounts.map((a) => [a.from_email.toLowerCase(), a]));
        const foundAccounts = csvEmails
            .map((email) => accountByEmail.get(email.toLowerCase()))
            .filter((a) => a !== undefined);
        const notFoundEmails = csvEmails.filter((email) => !accountByEmail.has(email.toLowerCase()));
        ...
        const reputationResults = await Promise.all(foundAccounts.map(async (account) => {
        ...
        const targetCampaigns = allCampaigns.items
            .filter((c) => {
            if (c.status !== 'PAUSED')
                return false;
        ...
        for (const campaign of targetCampaigns) {
            ...
            const action = this.describeAction(removeExistingMailboxes, existingIds.length, qualifiedIds.length);
            if (dryRun) {
    ...
    describeAction(removeExisting, existingCount, newCount) {
        if (existingCount === 0)
            return `Add ${newCount}`;
        if (removeExisting)
            return `Replace (remove ${existingCount}, add ${newCount})`;
        return `Add ${newCount} (keep ${existingCount} existing)`;
    }
    buildReport(params, mailboxStats, pausedInRange, campaigns, dryRun) {
    ...
    async parseCsvEmails(csvFilePath) {
