...
export class SmartleadClient {
    ...
    async getCampaignEmailAccounts(campaignId) {
        try {
            const body = await this.getJson(`/campaigns/${campaignId}/email-accounts`, new URLSearchParams());
            return Array.isArray(body) ? body : [];
        }
        catch (error) {
     ...
     */
    async getClientEmailAccounts(clientId) {
        try {
            const params = new URLSearchParams({ client_id: clientId });
            const body = await this.getJson(`/email-accounts`, params);
            return Array.isArray(body) ? body : [];
        }
        catch {
            return [];
     ...
     */
    async getEmailAccountWarmupStats(accountId) {
        try {
            const body = await this.getJson(`/email-accounts/${accountId}/warmup-stats`, new URLSearchParams());
    ...
    async addEmailAccountsToCampaign(campaignId, emailAccountIds) {
        try {
            await this.postJson(`/campaigns/${campaignId}/email-accounts`, { email_account_ids: emailAccountIds });
            return true;
        }
        catch (error) {
    ...
    async updateCampaignStatus(campaignId, status) {
        try {
            await this.postJson(`/campaigns/${campaignId}/update-status`, { status });
            return true;
        }
        catch (error) {
     ...
     */
    async deleteLeadFromCampaign(campaignId, leadId) {
        await this.campaignRateLimiter.throttle();
