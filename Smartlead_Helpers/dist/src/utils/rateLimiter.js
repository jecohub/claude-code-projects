...
export class RateLimiter {
    ...
    async throttle() {
        ...
        this.requestTimes = this.requestTimes.filter((time) => now - time < this.timeWindowMs);
        ...
        if (this.requestTimes.length >= this.maxRequests) {
            ...
            this.requestTimes = this.requestTimes.filter((time) => afterWait - time < this.timeWindowMs);
        }
        // Record this request
...
export class AdvancedRateLimiter extends RateLimiter {
    ...
    getStats() {
        return {
            ...
            throttleRate: this.requestCount > 0
