...
import { saveClientMapping, hasClientMapping, } from "../features/bulk-upload/utils/mappingStorage.js";
...
}).refine((data) => data.minReputation <= data.maxReputation, {
...
async function main() {
    ...
    server.registerTool("getClientStatus", {
    ...
    server.registerTool("getLeadStatusBreakdown", {
        ...
        const statusEntries = Object.entries(breakdown.statusBreakdown).sort(([, a], [, b]) => b - a);
    ...
    server.registerTool("getCampaignReport", {
    ...
    server.registerTool("bulkUploadLeads", {
        title: "Bulk upload leads with campaign duplication",
        description: "Split a large CSV file by ESP and email validity, duplicate campaigns for each split, and upload leads automatically. " +
    ...
    server.registerTool("previewCSVMapping", {
        title: "Preview CSV field mappings",
        description: "Analyze a CSV file and show auto-detected field mappings with sample data. " +
        ...
        textLines.push(`\n💡 NEXT STEPS:`);
        if (args.clientId) {
            textLines.push(`  1. Review the mappings above`);
            textLines.push(`  2. If corrections needed, call saveFieldMapping with corrected mappings`);
            textLines.push(`  3. Call bulkUploadLeads with isNewCampaign=true (first time) or isNewCampaign=false (use saved)`);
        }
        else {
            textLines.push(`  1. Provide a clientId parameter to save these mappings`);
            textLines.push(`  2. Call saveFieldMapping to store the confirmed mappings`);
        }
        const text = textLines.join("\n");
        return {
            structuredContent: {
    ...
    server.registerTool("saveFieldMapping", {
        title: "Save field mappings for a client",
        description: "Save confirmed field mappings for a Smartlead client. " +
        ...
        const mappings = args.mappings.map((m) => ({
            csvColumn: m.csvColumn,
            aliases: m.aliases,
            smartleadField: m.smartleadField,
        ...
        for (const mapping of mappings) {
            const fieldType = mapping.smartleadField === "custom"
    ...
    server.registerTool("swapCampaignMailboxes", {
        title: "Swap campaign mailboxes",
        description: "Replace or augment email accounts on paused Smartlead campaigns. " +
