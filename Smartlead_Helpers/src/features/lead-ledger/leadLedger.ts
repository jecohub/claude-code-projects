...
import type { LeadRow } from "../../core/types.js";

// Avoid a hard TS module dependency so `tsc --noEmit` can run even before `npm install`.
// Runtime still requires `better-sqlite3` to be installed.
...
function loadBetterSqlite3(): BetterSqlite3Constructor {
  ...
  cachedBetterSqlite3 = require("better-sqlite3") as BetterSqlite3Constructor;
  return cachedBetterSqlite3;
}

export type LeadLedgerLeadStatus = {
  invalidEmail?: boolean;
  unsubscribed?: boolean;
};

export type LeadLedgerUploadMeta = {
  ...
  apiInvalidEmailCount?: number;
  apiUnsubscribedCount?: number;
};

export type LeadLedgerEligibleLead = {
  email: string;
...
export class LeadLedger {
  ...
  static defaultDbPath(projectRoot: string): string {
  ...
  init(): CreateTablesResult {
    if (this.initialized) return { created: false };
  ...
  private ensureColumnExists(
    ...
    const cols = pragmaStmt.all() as Array<{ name?: unknown }>;
  ...
  bulkUpsertClients(clients: ClientRecord[]): { upserted: number } {
    this.init();
   ...
   */
  recordUpload(meta: LeadLedgerUploadMeta, leads: Array<{ email: string; row: LeadRow }>): number {
    this.init();
  ...
  upsertLeadStatus(email: string, status: LeadLedgerLeadStatus): void {
    this.init();
  ...
  bulkUpsertLeadStatus(
    emails: string[],
    status: LeadLedgerLeadStatus,
  ): { updated: number } {
    this.init();
  ...
  getEligibleLeads(params: {
    ...
    const excludeInvalid =
      params.includeInvalid ? "" : "AND COALESCE(ls.invalid_email, 0) = 0";
    const excludeUnsubscribed =
      params.includeUnsubscribed ? "" : "AND COALESCE(ls.unsubscribed, 0) = 0";
    ...
    return rows.map((r) => ({
      ...
      row: JSON.parse(r.row_json) as LeadRow,
    }));
  }
  ...
  exportEligibleToCsv(params: {
    ...
    includeInvalid?: boolean;
    limit?: number;
  }): { exported: number; outFilePath: string } {
