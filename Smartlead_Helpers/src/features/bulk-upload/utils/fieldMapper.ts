import { Lead, LeadRow, SmartleadFieldType, FieldMappingPreview } from "../../../core/types.js";
import { FieldMapping } from "./mappingStorage.js";
 ...
 */
export function detectFieldType(columnName: string): SmartleadFieldType {
  const normalized = columnName.toLowerCase().trim();
  const mappedField = FIELD_MAPPINGS[normalized];

  if (mappedField) {
    return mappedField as SmartleadFieldType;
  }
...
export function generateMappingPreview(
  ...
  const previews: FieldMappingPreview[] = [];
 ...
 */
function findColumnValue(row: LeadRow, mapping: FieldMapping): string | undefined {
