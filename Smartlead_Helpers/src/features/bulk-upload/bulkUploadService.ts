...
export class BulkUploadService {
  constructor(private client: SmartleadClient) {}

  private leadLedger: LeadLedger | null | undefined;

  /**
   * Best-effort local lead ledger (never blocks uploads).
   */
  private getLeadLedger(): LeadLedger | null {
    // tri-state: undefined = not attempted, null = unavailable, instance = ready
    if (this.leadLedger !== undefined) return this.leadLedger;

    try {
      const dbPath =
        process.env.LEAD_LEDGER_DB_PATH ||
   ...
   */
  async execute(params: BulkUploadParams): Promise<BulkUploadResult> {
    const errors: string[] = [];
    const campaignResults: CampaignUploadResult[] = [];
    ...
    try {
      ...
      if (!params.isNewCampaign && params.clientId) {
        ...
        errors.push(
          `Warning: isNewCampaign=true but no clientId provided. Using auto-detection. ` +
      ...
      const campaignMappings = new Map<
        string,
        { campaignId: number; errors: string[]; warnings: string[] }
      >();
      ...
      for (let i = 0; i < splits.length; i += concurrency) {
        ...
        const duplicationPromises = batch.map(async (split) => {
          try {
            console.log(
              `Duplicating campaign for ${split.groupType} split ${split.splitNumber}...`,
            );

            const duplicationResult = await this.client.duplicateCampaign(
              params.sourceCampaignId,
              split.campaignName,
              params.clientId,
              {
            ...
            if (!duplicationResult.success) {
              duplicationErrors.push(
                ...duplicationResult.errors.map((e) => `Duplication: ${e}`),
              );
            }

            console.log(
              `  ✓ Campaign ${duplicationResult.campaignId} created for ${split.groupType} split ${split.splitNumber}`,
            );
            ...
            console.error(
              `  ❌ Failed to duplicate campaign for "${split.campaignName}": ${errorMessage}`,
            );
      ...
      for (const split of splits) {
        ...
        try {
          ...
          const result = await this.uploadLeadsToExistingCampaign(
            mapping.campaignId,
            split,
            params.ignoreGlobalBlockList,
            mappings,
            params.csvFilePath,
            params.clientId,
          );
      ...
      console.log(
        `Campaign Operations: ${stats.campaignOps.totalRequests} requests (${stats.campaignOps.throttleRate.toFixed(1)}% throttled)`,
      );
      console.log(
        `Lead Uploads: ${stats.leadUploads.totalRequests} requests (${stats.leadUploads.throttleRate.toFixed(1)}% throttled)`,
      );
      console.log(
        `Read Operations: ${stats.readOps.totalRequests} requests (${stats.readOps.throttleRate.toFixed(1)}% throttled)`,
      );
   ...
   */
  private async uploadLeadsToExistingCampaign(
    campaignId: number,
    split: LeadSplit,
    ...
    try {
      const uploadResponse = await this.client.addLeadsToCampaign(
        campaignId,
        leads,
        {
      ...
      try {
        ...
        if (ledger) {
          const rowByEmail = new Map<string, (typeof split.leads)[number]>();
          for (const row of split.leads) {
            const mapped = mapCSVRowToLead(row, mappings);
            const email = mapped.email?.toLowerCase().trim();
            if (!email) continue;
            if (!rowByEmail.has(email)) rowByEmail.set(email, row);
          }

          const emailsToRecord =
            uploadResponse.ledger_new_lead_emails?.length
