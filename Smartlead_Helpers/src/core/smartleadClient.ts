import { SmartleadConfig, RATE_LIMITS, CONCURRENCY_LIMITS } from "./config.js";
import {
...
export class SmartleadClient {
  ...
  private readonly campaignRateLimiter: AdvancedRateLimiter;
  private readonly leadUploadRateLimiter: AdvancedRateLimiter;
  private readonly readRateLimiter: AdvancedRateLimiter;
  ...
  constructor(config: SmartleadConfig) {
    ...
    this.campaignRateLimiter = new AdvancedRateLimiter(
      RATE_LIMITS.CAMPAIGN_OPERATIONS.maxRequests,
      RATE_LIMITS.CAMPAIGN_OPERATIONS.timeWindowMs,
    );

    this.leadUploadRateLimiter = new AdvancedRateLimiter(
      RATE_LIMITS.LEAD_UPLOADS.maxRequests,
      RATE_LIMITS.LEAD_UPLOADS.timeWindowMs,
    );

    this.readRateLimiter = new AdvancedRateLimiter(
      RATE_LIMITS.READ_OPERATIONS.maxRequests,
      RATE_LIMITS.READ_OPERATIONS.timeWindowMs,
    );
  }
  ...
  async getCampaignLeads(campaignId: number): Promise<any[]> {
    try {
      ...
      const firstPage = await this.getJson<any>(
        `/campaigns/${campaignId}/leads`,
        params,
      );
      ...
      while (hasMore && allLeads.length < 100000) {
        ...
        const requestBatch: Promise<any>[] = [];
        ...
        for (let i = 0; i < concurrency && hasMore; i++) {
          ...
          requestBatch.push(
            this.getJson<any>(`/campaigns/${campaignId}/leads`, params).catch(
              (error) => {
   ...
   */
  private readonly categoryIdMap: Record<number, keyof LeadCategorization> = {
    1: "interested",
    2: "meetingRequest",
    3: "notInterested",
    4: "doNotContact",
   ...
   */
  async getLeadCategorization(campaignId: number): Promise<LeadCategorization> {
    const categorization: LeadCategorization = {
      interested: 0,
      meetingRequest: 0,
  ...
  private async postJson<T>(
    ...
    body: unknown,
    params?: URLSearchParams,
    isLeadUpload = false, // NEW PARAMETER
  ): Promise<T> {
  ...
  private getGraphqlAuthHeader(tokenOverride?: string): string | undefined {
  ...
  private async postGraphql<TData>(
    request: { operationName: string; variables: unknown; query: string },
    tokenOverride?: string,
  ): Promise<TData> {
    const auth = this.getGraphqlAuthHeader(tokenOverride);
    if (!auth) {
      throw new Error(
        "Missing Smartlead web auth token. Set SMARTLEAD_WEB_AUTH_TOKEN or pass uiAuthToken to enable UI-only settings copy.",
      );
    }
    ...
    const text = await response.text();
    let envelope: GraphqlEnvelope<TData> | undefined;
    try {
      envelope = JSON.parse(text) as GraphqlEnvelope<TData>;
    } catch {
    ...
    if (!response.ok) {
      const details = envelope?.errors?.length
        ...
        : text;
      throw new Error(
        `Smartlead GraphQL request failed (${response.status}): ${details}`,
      );
    }

    if (!envelope) {
      throw new Error(
        `Smartlead GraphQL returned non-JSON response (${response.status}): ${text.slice(0, 500)}`,
      );
    }

    if (envelope.errors?.length) {
      throw new Error(
        `Smartlead GraphQL errors: ${envelope.errors.map((e) => e.message).join("; ")}`,
      );
    }
   ...
   */
  private async applyUiOnlySettingsGraphql(
    sourceCampaignId: number,
    targetCampaignId: number,
    tokenOverride?: string,
  ): Promise<{ applied: boolean; details: string[] }> {
    type UiOnlySettings = {
      ai_categorisation_options?: unknown;
      bounce_autopause_threshold?: number | null;
      out_of_office_detection_settings?: unknown | null;
    };

    type GetSettingsData = {
      email_campaigns_by_pk: UiOnlySettings | null;
    };
    ...
    const settingsData = await this.postGraphql<GetSettingsData>(
      {
    ...
    type UpdateData = {
      update_email_campaigns_by_pk: { id: number } | null;
    };
    ...
    await this.postGraphql<UpdateData>(
      {
  ...
  async createCampaign(
    ...
    return this.postJson<CampaignCreateResponse>("/campaigns/create", request);
  }
  ...
  async saveCampaignSequences(
    campaignId: number,
    sequences: CampaignSequenceResponse[]
  ): Promise<void> {
    ...
    for (let i = 0; i < batches.length; i += concurrency) {
      ...
      const uploadPromises = chunk.map(async (batch, chunkIndex) => {
        ...
        const request: AddLeadsRequest = {
          ...
          settings: {
            ...
            ignore_duplicate_leads_in_other_campaign:
              options?.ignoreDuplicateLeadsInOtherCampaign,
          },
        };
        ...
        while (retries < maxRetries) {
          try {
            const response = await this.postJson<AddLeadsResponse>(
              `/campaigns/${campaignId}/leads`,
              request,
              undefined,
              true, // isLeadUpload = true (uses lead upload rate limiter)
            );
      ...
      // Progress indicator
      console.log(
        `    Chunk ${chunkNumber}/${totalChunks}: ${results.totalUploaded} leads uploaded so far`,
      );
    }
  ...
  async addLeadsToCampaign(
    ...
    const existingEmails = new Set(
      existingLeads
        .map((item: any) => item.lead?.email || item.email)
    ...
    console.log(
      `     Processing ${batches.length} batches with ${CONCURRENCY_LIMITS.BATCH_UPLOADS}x concurrency`,
    );

    // Upload batches in parallel
    const results = await this.uploadBatchesInParallel(
      campaignId,
      batches,
    ...
    if (results.failedBatches.length > 0) {
      ...
      const retryResults = await this.uploadBatchesInParallel(
        campaignId,
      ...
      if (retryResults.failedBatches.length > 0) {
        console.error(
          `     ❌ ${retryResults.failedBatches.length} batches still failed after retry`,
        );
      } else {
    ...
    console.log(
      `  ✓ Upload complete: ${results.totalUploaded}/${newLeads.length} new leads uploaded`,
    );
    if (alreadyUploaded > 0) {
   ...
   */
  private async validateSourceCampaign(
    campaignId: number
   ...
   */
  private async verifyCampaignCopy(
    sourceCampaignId: number,
    newCampaignId: number,
    expectedSequences: number
  ): Promise<DuplicateCampaignResult['verification']> {
    const details: string[] = [];
    ...
    try {
      ...
      const settingsMatch =
        JSON.stringify(newDetails.track_settings) === JSON.stringify(sourceDetails.track_settings) &&
      ...
      // Check schedule
      const scheduleMatch = !!(
        newDetails.scheduler_cron_value &&
      ...
      if (!sequenceCountMatch) {
        details.push(
          `Expected ${expectedSequences} sequences, found ${newSequences.length}`
        );
      }
   ...
   */
  private sanitizeSequence(seq: any): any {
    const sanitized: any = {
    ...
    if (seq.sequence_variants && Array.isArray(seq.sequence_variants)) {
      sanitized.seq_variants = seq.sequence_variants.map((variant: any) => ({
  ...
  async duplicateCampaign(
    ...
    // Apply defaults
    const opts: Required<DuplicateCampaignOptions> = {
    ...
    try {
      ...
      steps.push(this.createStep(
        ...
        'success',
        stepStart,
        `Source campaign validated (${validation.sequenceCount} sequences)`
      ));
      ...
      stepStart = Date.now();
      const fetchResult = await this.retryOperation(
        async () => ({
      ...
      steps.push(this.createStep(
        ...
        'success',
        stepStart,
        `Fetched ${sourceSequences.length} sequences`
      ));
      ...
      stepStart = Date.now();
      const createResult = await this.retryOperation(
        () => this.createCampaign({
      ...
      if (!opts.skipSettings) {
        ...
        let trackSettings: unknown = sourceDetails.track_settings;
        if (Array.isArray(trackSettings) && trackSettings.length > 0) {
          ...
          trackSettings = trackSettings
            ...
            .filter((val: any) => val !== null && val !== ''); // Remove nulls and empty strings

          if ((trackSettings as any[]).length === 0) {
        ...
        const settings: CampaignSettings = {
          ...
          unsubscribe_text: sourceDetails.unsubscribe_text as string | undefined,
          send_as_plain_text: sourceDetails.send_as_plain_text as boolean | undefined,
          follow_up_percentage: sourceDetails.follow_up_percentage as number | undefined,
          enable_ai_esp_matching: sourceDetails.enable_ai_esp_matching as boolean | undefined,
        };

        const settingsResult = await this.retryOperation(
          () => this.updateCampaignSettings(newCampaignId, settings),
          'copy_settings',
          opts.retryAttempts,
          opts.retryDelayMs,
          opts.verbose
        ...
        steps.push(this.createStep(
          ...
          'skipped',
          Date.now(),
          'Settings copy skipped by configuration'
        ));
      }
      ...
      if (!opts.skipSchedule && sourceDetails.scheduler_cron_value) {
        ...
        const scheduleResult = await this.retryOperation(
          () => this.updateCampaignSchedule(newCampaignId, schedule),
          'copy_schedule',
          opts.retryAttempts,
          opts.retryDelayMs,
          opts.verbose
        ...
        steps.push(this.createStep(
          ...
          'skipped',
          Date.now(),
          'Schedule copy skipped by configuration'
        ));
      } else {
        steps.push(this.createStep(
          'copy_schedule',
          'skipped',
          Date.now(),
          'No schedule configured in source campaign'
        ));
      }
      ...
      if (!opts.skipSequences && sourceSequences.length > 0) {
        ...
        const sequencesResult = await this.retryOperation(
          () => this.saveCampaignSequences(newCampaignId, sanitizedSequences),
          'copy_sequences',
          opts.retryAttempts,
          opts.retryDelayMs,
          opts.verbose
        ...
        if (sequencesResult.success) {
          ...
          steps.push(this.createStep(
            ...
            'success',
            stepStart,
            `Copied ${sourceSequences.length} sequences`
          ));
        } else {
          steps.push(this.createStep(
            'copy_sequences',
        ...
        steps.push(this.createStep(
          ...
          'skipped',
          Date.now(),
          'Sequences copy skipped by configuration'
        ));
      } else {
        steps.push(this.createStep(
          'copy_sequences',
          'skipped',
          Date.now(),
          'No sequences configured in source campaign'
        ));
      }
      ...
      // STEP 7: Apply UI-only settings (AI/bounce from config, OOO from source)
      {
        ...
        if (opts.skipUiOnlySettings) {
          steps.push(this.createStep(
            'apply_ui_only_settings',
            'skipped',
            uiStepStart,
            'UI-only settings application skipped by configuration'
          ));
          ...
          steps.push(this.createStep(
            ...
            'skipped',
            uiStepStart,
            'Missing SMARTLEAD_WEB_AUTH_TOKEN (web auth token not provided)'
          ));
          warnings.push(
            'UI-only settings (AI categorisation, bounce auto-protection, OOO settings) not applied: missing SMARTLEAD_WEB_AUTH_TOKEN',
          );
        } else {
          if (opts.verbose) {
          ...
          const uiResult = await this.retryOperation(
            () => this.applyUiOnlySettingsGraphql(sourceCampaignId, newCampaignId, opts.uiAuthToken),
            'apply_ui_only_settings',
            opts.retryAttempts,
            opts.retryDelayMs,
            opts.verbose
          ...
          if (uiResult.success && uiResult.result) {
            ...
            steps.push(this.createStep(
              'apply_ui_only_settings',
              uiResult.result.applied ? 'success' : 'skipped',
              uiStepStart,
              uiResult.result.details.join('; ')
            ));
      ...
      let verification: DuplicateCampaignResult['verification'] | undefined;
      ...
      if (opts.verifyAfterCopy) {
        ...
        stepStart = Date.now();
        verification = await this.verifyCampaignCopy(
          sourceCampaignId,
          newCampaignId,
        ...
        if (verification && verification.verified) {
          steps.push(this.createStep(
            ...
            'success',
            stepStart,
            'Campaign copy verified successfully'
          ));
        } else {
          steps.push(this.createStep(
            'verify_copy',
      ...
      const success = errors.length === 0 && (
        !opts.verifyAfterCopy || (verification?.verified === true)
      );
