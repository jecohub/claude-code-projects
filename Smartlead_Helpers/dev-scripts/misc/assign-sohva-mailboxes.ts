...
async function main() {
  ...
  console.log(`Fetching all Sohva email accounts...`);
  const allAccounts = await getAllEmailAccounts(CLIENT_ID);
  console.log(`Found ${allAccounts.length} total accounts in Smartlead`);
  ...
  const unmatched = csvEmails.filter(e => !matched.some((a: any) => a.from_email?.toLowerCase() === e));
  ...
  if (matchedIds.length === 0) {
    console.error("\nNo accounts matched. Aborting.");
    process.exit(1);
  }

  console.log(`\nFetching Sohva campaigns...`);
  const result = await client.listCampaigns(CLIENT_ID);
  ...
  for (const campaign of campaigns) {
    ...
    if (toAdd.length === 0) {
      console.log(`  Campaign "${campaignName}" (${campaignId}): all mailboxes already assigned ✓`);
      continue;
    }

    process.stdout.write(`  Campaign "${campaignName}" (${campaignId}): adding ${toAdd.length} mailboxes... `);
